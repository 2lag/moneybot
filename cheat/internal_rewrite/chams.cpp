...
namespace features
{
	void c_materials::make_cham_buffer( char* buf, int len, int type ) {
		switch ( type ) {
		case 1:
			sprintf_s( buf, len - 1, chams_mat_flat );
			break;
		case 2:
			sprintf_s( buf, len - 1, chams_mat );
			break;
		default:
			sprintf_s( buf, len - 1, chams_mat );
		}
	...
	__declspec( safebuffers ) void c_materials::initialize_materials( ) {
		char buffer[ 1024 ];
