...
std::shared_ptr< ui::c_menu > ui::menu = std::make_shared< ui::c_menu >( 10, 10, 580, 470, xors( "i will skull fuck you skid" ),
	"" );

int ui::current_category = 0;
bool ui::was_setup = false;
int ui::theme = 0;
...
void on_d3d( IDirect3DDevice9* device ) {
	...
	if( g_settings.menu.open || !ui::was_setup ) {
		if( anim_time >= 0.95f ) {
			flip = true;
		}
		if( anim_time <= 0.05f ) {
		...
		ui::theme = g_settings.menu.theme;
		ui::render( );
	}
	else {
		anim_progress = 0;
