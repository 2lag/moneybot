...
namespace features
{
	...
	private:
		...
		using target_t = std::pair< int, vec3_t >;
		...
		void	aim_at_target( target_t ent, float*, float* );

		bool	 update_settings( );
		float	 get_avg_delta( );
		target_t get_aim_target( float fov = 0.f );
		int		 get_bone( c_base_player* ent, c_legitbot::lag_record_t* lag_record );
